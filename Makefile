...
OPTIONS ?=
finetune-x1:
	tune run recipes/lora_finetune_single_device.py --config config/8B_lora.yaml \
		checkpointer.output_dir=$(CKPT_OUTPUT) $(OPTIONS)

finetune-x%:
	tune run --nnodes 1 --nproc_per_node $* \
		recipes/lora_finetune_distributed.py --config config/8B_lora.yaml \
		checkpointer.output_dir=$(CKPT_OUTPUT) $(OPTIONS) \
		gradient-accumulation-steps=32

# eg. make eval_baseline, or make eval_finetune
eval%:
	tune run recipes/eleuther_eval.py --config config/eleuther_eval$*.yaml

mm_eval:
	tune run recipes/mm_eval.py --config config/mm_eval.yaml

# included as an example: replace prompt as required
gen:
	tune run recipes/gen.py --config config/gen.yaml prompt="definition of inference in one word"

# included as an example: replace prompt as required
mmgen:
	tune run recipes/gen.py --config config/gen.yaml image="media/cactus.png" prompt="Caption the image\nImage:\n{image}"
	# tune run recipes/gen.py --config config/gen.yaml image="media/cactus.png" prompt="Image:\n{image}\nCaption the preceding image."
